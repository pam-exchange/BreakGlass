...
<#
    switch ($PSCmdlet.MyInvocation.MyCommand.Name) {
    "Test-KeePassXCGroup" {
    }
    default { } # optional
    }
    ...
    if ($msg -match 'Group .* already exists')
    {
        #
        # mkdir
        #
    ...
    if ($msg -match 'Cannot find group')
    {
        #
        # ls 
        #
    ...
    if ($msg -match 'Could not create entry with path')
    {
        #
        # add
        #
    ...
    if ($msg -match 'Missing positional argument')
    {
        #
        # mkdir without group
        #
    ...
    if ($msg -match 'Invalid credentials were provided')
    {
        #
        # Wrong MasterPassword for database
        #
    ...
    if ($msg -match 'Failed to load key file')
    {
        #
        # Keyfile not found
        #
    ...
    if ($msg -match 'Failed to open database file')
    {
        #
        # Database is not found
        #
    ...
    if ($msg -match 'Not a KeePass database')
    {
        #
        # Database file is not KeePassXC file format
        #
